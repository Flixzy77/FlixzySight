...
class CrosshairOverlay(ctk.CTkToplevel):
    ...
    def set_crosshair_from_image(self, pil_image):
        final_size = (100, 100)
        resized_image = pil_image.resize(final_size, Image.Resampling.NEAREST)
        background = Image.new("RGB", final_size, self.transparent_color)
        background.paste(resized_image, (0, 0), resized_image)
        ctk_image = ctk.CTkImage(light_image=background, dark_image=background, size=final_size)
        self.update_image(ctk_image)
        return ctk_image
    ...
    def _draw_single_crosshair(self, draw_context, params, color, offset=(0, 0)):
        ...
        if params.get("inner_lines_enabled", True):
            l, t, o = (params.get(f"inner_lines_{k}", 2.0) for k in ["length", "thickness", "offset"])
            ht = t / 2.0
    ...
    def draw_crosshair_from_params(self, params):
        ...
        for key in ["center_dot_size", "inner_lines_length", "inner_lines_thickness", "inner_lines_offset"]:
            if key in scaled_params: scaled_params[key] *= scale
        ...
        # *** OSTATECZNA POPRAWKA: ZAWSZE UŻYWAMY FILTRA "NEAREST" ***
        # Usuwamy całą logikę warunkową i na stałe ustawiamy renderowanie pikselowe.
    ...
    def center_window(self):
        ...
        x, y = (w // 2) - (self.winfo_width() // 2), (h // 2) - (self.winfo_height() // 2)
        self.geometry(f"+{x}+{y}")
