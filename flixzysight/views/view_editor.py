...
# --- Definicja kolorów ---
ACCENT_COLOR = "#9370DB"  # MediumPurple
FRAME_COLOR = "#242424"
BG_COLOR = "#1B1B1B"
HOVER_COLOR = "#333333"
...
class EditorView(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.app = parent
        self.params = {}
        self.profiles_dir = "assets/presets"
        ...
        self.profiles_frame = self.create_section_frame(self, "Profiles")
        ...
        self.create_header(self.controls_scroll_frame, "General")
        self.controls['color'] = self.create_color_picker(self.controls_scroll_frame, "Color", icon_char="🎨")
        self.controls['outline_enabled'] = self.create_switch(self.controls_scroll_frame, "Outlines", icon_char="🔳")
        # *** ZMIANA: Usunięcie "is_int=True" ***
        self.controls['outline_thickness'] = self.create_slider(self.controls_scroll_frame, "Outline Thickness", 0, 5, icon_char="↔")

        self.create_header(self.controls_scroll_frame, "Center Dot")
        self.controls['center_dot_enabled'] = self.create_switch(self.controls_scroll_frame, "Enabled", icon_char="●")
        self.controls['center_dot_size'] = self.create_slider(self.controls_scroll_frame, "Size", 0, 10, icon_char="↔")

        self.create_header(self.controls_scroll_frame, "Inner Lines")
        self.controls['inner_lines_enabled'] = self.create_switch(self.controls_scroll_frame, "Enabled", default_on=True, icon_char="╋")
        self.controls['inner_lines_length'] = self.create_slider(self.controls_scroll_frame, "Length", 1, 20, icon_char="↕")
        self.controls['inner_lines_thickness'] = self.create_slider(self.controls_scroll_frame, "Thickness", 1, 10, icon_char="↔")
        self.controls['inner_lines_offset'] = self.create_slider(self.controls_scroll_frame, "Offset", 0, 20, icon_char="✥")
    ...
    def create_section_frame(self, parent, title):
        ...
        label = ctk.CTkLabel(frame, text=title, font=ctk.CTkFont(size=14, weight="bold"), anchor="w")
        label.pack(fill="x", padx=15, pady=(8, 4))
        return frame

    def create_header(self, parent, text):
        return ctk.CTkLabel(parent, text=text, font=ctk.CTkFont(size=13, weight="bold"), anchor="w").pack(fill="x", padx=10, pady=(15, 5))
    ...
    def create_slider(self, parent, label_text, from_, to, icon_char=None, is_int=False):
        ...
        if icon_char:
            icon = ctk.CTkLabel(frame, text=icon_char, font=ctk.CTkFont(size=18), width=20)
            icon.pack(side="left", padx=(0, 8))
            
        label = ctk.CTkLabel(frame, text=label_text, width=120, anchor="w").pack(side="left")
        slider = ctk.CTkSlider(frame, from_=from_, to=to, button_color=ACCENT_COLOR, progress_color=ACCENT_COLOR)
        slider.pack(side="left", fill="x", expand=True, padx=10)
        
        format_str = "{:.0f}" if is_int else "{:.2f}"
        value_label = ctk.CTkLabel(frame, text=format_str.format(slider.get()), width=40)
        value_label.pack(side="right")
        
        slider.configure(command=lambda v, l=value_label: (l.configure(text=format_str.format(v)), self.update_crosshair()))
        return slider
    ...
    def create_color_picker(self, parent, label_text, icon_char=None):
        ...
        if icon_char:
            ...
            icon.pack(side="left", padx=(0, 8))
        label = ctk.CTkLabel(frame, text=label_text, width=120, anchor="w").pack(side="left")
        color_button = ctk.CTkButton(frame, text="", width=32, height=32, fg_color=ACCENT_COLOR, command=self.pick_color, hover_color=ACCENT_COLOR)
    ...
    def update_crosshair(self, *_):
        ...
        if not self.app.overlay_window.is_visible:
            ...
            if 'overlay' in self.app.views: self.app.views['overlay'].update_switch_state()
        ...
        try:
            for filename in sorted(os.listdir(self.profiles_dir)):
                if filename.endswith(".json"):
                    ...
                    profile_entry = ctk.CTkFrame(self.profiles_scroll_frame, fg_color="transparent", corner_radius=6)
                    profile_entry.pack(fill="x", padx=5, pady=3)
                    thumbnail_image = self.app.overlay_window.draw_crosshair_from_params(params)
                    self.thumbnail_references.append(thumbnail_image)
                    thumbnail_label = ctk.CTkLabel(profile_entry, image=thumbnail_image, text="", width=40, height=40, fg_color=BG_COLOR)
                    thumbnail_label.pack(side="left", padx=5, pady=5)
                    name_label = ctk.CTkLabel(profile_entry, text=name, anchor="w")
                    name_label.pack(side="left", expand=True, fill="x", padx=10)
                    profile_entry.bind("<Enter>", lambda e, w=profile_entry: w.configure(fg_color=HOVER_COLOR))
                    profile_entry.bind("<Leave>", lambda e, w=profile_entry: w.configure(fg_color="transparent"))
                    profile_entry.bind("<Button-1>", lambda e, p=path: self.load_profile(p))
                    name_label.bind("<Button-1>", lambda e, p=path: self.load_profile(p))
                    thumbnail_label.bind("<Button-1>", lambda e, p=path: self.load_profile(p))
    ...
    def load_default_profile(self):
        ...
        if not os.path.exists(default_file):
            default_params = {"color": ACCENT_COLOR, "outline_enabled": True, "outline_thickness": 1.0, "center_dot_enabled": False, "center_dot_size": 2.0, "inner_lines_enabled": True, "inner_lines_length": 6.0, "inner_lines_thickness": 2.0, "inner_lines_offset": 3.0}
            with open(default_file, 'w') as f: json.dump(default_params, f, indent=4)
        self.load_profile(default_file)
    ...
    def save_profile(self):
        ...
        if profile_name:
            file_path = os.path.join(self.profiles_dir, f"{profile_name.strip()}.json")
            try:
                with open(file_path, 'w') as f: json.dump(self.params, f, indent=4)
    ...
    def import_profile(self):
        ...
        if code:
            try:
                decoded_string = base64.b64decode(code).decode('utf-8')
                params = json.loads(decoded_string)
